...
Decision: ONNX Runtime is the local ML runtime for both the bundled LLM and embedding model. The bundled LLM is **Gemma 4 E2B IT QAT mobile** (`q2f16` text graphs with external data); the embedder is **EmbeddingGemma 300M Q4** (`model_q4.onnx` plus external data). Both use pinned manifests, verified downloads, and one prioritized worker so only one local model is active at a time.
