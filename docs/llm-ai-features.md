...
The bundled model is **[EmbeddingGemma 300M](https://huggingface.co/onnx-community/embeddinggemma-300m-ONNX)**
with Q4 weights and 256-dimensional MRL output. It has distinct retrieval query/document prompts and a
2,048-token context. Canonical vectors are generated by the serial local-AI worker, synced as per-note binary
sidecars, and projected into local SQLite. A device may disable background note generation while retaining
synced vectors and query-only inference.

The model is used purely as a **dense** embedder. The sparse / BM25 arm of search comes from SQLite **FTS5**
([search.md](search.md)), not the embedding model, so a model with no native sparse output loses
nothing; learned-sparse heads from other embedders are optional bonuses the pipeline does not depend on. Vectors are
stored in synced `_embeddings/` sidecars and materialized into SQLite under `_derived/`; `sqlite-vec` remains the intended acceleration layer once a
...
- One vector for the **full note**
- Long full notes retain exact tokenizer IDs from the beginning and end, sharing the remaining 2,048-token
  budget after the document prompt and title.
