...
/// List every installed plugin for the UI.
pub fn list_plugins(registry: &PluginRegistry) -> Vec<PluginDescriptor> {
    registry
        .all()
        .iter()
...
mod run {
    ...
    /// Render a fenced code block with the first plugin that claims its language; returns the
    /// plugin's (unsanitized) HTML. The caller sanitizes before display.
